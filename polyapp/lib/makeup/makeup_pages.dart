...
class _MakeupWorkspacePageState extends State<MakeupWorkspacePage> {
  ...
  Future<void> _createCase() async {
    if (_group == null || _studentId == null || _classDates.isEmpty) return;
    try {
...
class _AdminWorkspacePageState extends State<AdminWorkspacePage> {
  ...
  void _reloadAll() {
    ...
    _teachersFuture = widget.client.listUsers(role: 'teacher');
    ...
    if (_journalGroup != null && _journalGroup!.trim().isNotEmpty) {
      _journalStudentsFuture = widget.client.listJournalStudents(
        _journalGroup!,
      );
      _journalDatesFuture = widget.client.listJournalDates(_journalGroup!);
    } else {
          ...
          child: Column(
            ...
            children: [
              ...
              TextField(
                ...
                decoration: InputDecoration(labelText: _t('ФИО', 'Full name')),
              ),
              const SizedBox(height: 8),
              TextField(
  ...
  Future<void> _editUser(UserProfile user) async {
          ...
          child: Column(
            ...
            children: [
              ...
              TextField(
                ...
                decoration: InputDecoration(labelText: _t('Телефон', 'Phone')),
              ),
              const SizedBox(height: 8),
              TextField(
    ...
    await widget.client.updateUser(user.id, {
      ...
      'student_group': groupController.text.trim(),
  ...
  Future<void> _createAssignment() async {
    final teachers =
        await (_teachersFuture ?? widget.client.listUsers(role: 'teacher'));
  ...
  Future<void> _addJournalStudent() async {
    ...
    await widget.client.upsertJournalStudent(
      ...
      studentName: controller.text.trim(),
    );
    _reloadAll();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return ModulePanel(
      title: _t('Админ панель', 'Admin panel'),
      subtitle: _t(
        'Управление пользователями, заявками и отработками.',
        'Manage users, requests, makeups and journal data.',
      ),
      trailing: () {
        if (_tab == 0) {
        ...
        children: [
          Card(
            child: Padding(
              ...
              child: Text(
                _t(
                  'CRUD доступ: пользователи, заявки, отработки, назначения преподавателей, группы/студенты/даты журнала.',
                  'CRUD scope: users, requests, makeups, teacher assignments, journal groups/students/dates.',
                ),
              ),
            ),
          ),
                                  ...
                                  child: DropdownButtonFormField<String>(
                                    ...
                                    onChanged: (value) {
                                      ...
                                      setState(() {
                                        _journalGroup = value;
                                        _journalStudentsFuture = widget.client
                                            .listJournalStudents(value);
                                        _journalDatesFuture = widget.client
                                            .listJournalDates(value);
                                      });
                                    },
                                    decoration: InputDecoration(
                                      labelText: _t('Группа', 'Group'),
